...
const SOLUTION_CODE = `export function hello(name: string): string {
    return \`Hello, \${name}!\`;
}`;

test.describe('world-ts — ts-ignition', () => {
    test.setTimeout(120_000);

    test('TypeScript Ignition end-to-end', async ({ page }) => {
        await page.addInitScript(() => {
            localStorage.setItem('evalforge:seenTutorial', '1');
        });

        // 1. Navigate to quest IDE
        await page.goto(`${BASE_URL}/arcade/worlds/world-ts/quests/ts-ignition`);
        // handle login
