...
const SOLUTION_CODE = `SELECT * FROM users ORDER BY id ASC;
`;

test.describe('world-sql — sql-ignition', () => {
    test.setTimeout(120_000);

    test('SQL Ignition end-to-end', async ({ page }) => {
        await page.addInitScript(() => {
            localStorage.setItem('evalforge:seenTutorial', '1');
        });

        // 1. Navigate to quest IDE
        await page.goto(`${BASE_URL}/arcade/worlds/world-sql/quests/sql-ignition`);
        // handle login
