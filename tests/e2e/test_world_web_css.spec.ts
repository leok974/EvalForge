...
test.describe('world-web css — css-ignition', () => {
    test.setTimeout(120_000);

    test('CSS Ignition: Your First Styles end-to-end', async ({ page }) => {
        await page.addInitScript(() => {
            localStorage.setItem('evalforge:seenTutorial', '1');
        });

        // 1. Navigate to quest IDE
        await page.goto(`${BASE_URL}/arcade/worlds/world-web/quests/css-ignition`);
        // handle login
        ...
        // 4. Replace with solution (style.css is the active entrypoint for css quest)
