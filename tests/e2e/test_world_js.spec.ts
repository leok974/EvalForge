...
const SOLUTION_CODE = `export function hello() {
    return 'Hello World';
}`;

test.describe('world-js — js-ignition-q1-console-and-functions', () => {
    test.setTimeout(120_000);

    test('Console and Functions end-to-end', async ({ page }) => {
        await page.addInitScript(() => {
            localStorage.setItem('evalforge:seenTutorial', '1');
        });

        // 1. Navigate to quest IDE
        await page.goto(`${BASE_URL}/arcade/worlds/world-js/quests/js-ignition-q1-console-and-functions`);
        // handle login
        ...
        // 3. Run starter code
        // NOTE: js-ignition starter code (export function task() {}) trivially satisfies
        // both objectives (exit_code_zero + source_regex "function"), so Submit may already
        // be enabled after the first run. We do NOT assert it is disabled here.
