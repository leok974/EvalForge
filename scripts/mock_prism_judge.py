...
def run_mock_submission():
    ...
    # 2. Load Rubric
    rubric_path = base_dir / "rubrics/boss-prism-refraction-type-guardian.json"
    print(f"Loading Rubric from {rubric_path}...")
    boss_rubric = load_json(rubric_path)
    
    # 3. Load Codex
    codex_path = base_dir / "codex/world-typescript/track-refraction-boss-type-guardian.md"
    print(f"Loading Codex from {codex_path}...")
    codex_content = load_file(codex_path)
    boss_codex = {
        "title": "Codex: Type Guardian",
        "content": codex_content
    }
    
    # 4. Load Solution (Submission)
    sol_dir = base_dir / "solutions/bosses/prism_refraction"
    
    files = []
    for fname in ["auth.ts", "auth.test.ts"]:
        fpath = sol_dir / fname
        if not fpath.exists():
    ...
    submission = {
        "files": files,
        "notes": "Here is my submission for the Type Guardian boss. Strict TypeScript implementation with discriminated unions."
    }
    ...
    boss_definition = next(
        (b for b in all_bosses["bosses"] if b["slug"] == "type-guardian"), 
        None
    )
    
    if not boss_definition:
        print("❌ Could not find 'type-guardian' boss in definition file.")
        return
